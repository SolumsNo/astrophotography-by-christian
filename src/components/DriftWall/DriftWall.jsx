...
const DriftWall = ({
  ...
  const baseVelocities = useMemo(() => {
    // Keep every column drifting in the selected direction. A small
    // per-column variance preserves the organic React Bits feel without
    // making neighbouring columns travel against each other.
    const dirSign = direction === 'up' ? 1 : -1;
    return columnItems.map((_, c) => speed * columnFactor(c, variance) * dirSign);
    ...
    const animate = ts => {
      ...
      if (!reduced) {
        for (let c = 0; c < trackRefs.current.length; c++) {
          ...
          // Hovering a tile may lift/highlight it, but the wall keeps moving
          // unless pauseOnHover has explicitly been enabled.
